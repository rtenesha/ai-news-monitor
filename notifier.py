...
def generate_post(article: dict) -> str:
    ...
    prompt = (
        ...
        "- Последняя строка — один короткий вопрос, который цепляет личный опыт читателя: "
        "не «что вы думаете?», а конкретный: «вы уже пробовали?», «как это меняет ваш процесс?», «стоит ли доверять?»\n"
