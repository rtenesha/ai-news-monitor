...
_ENGAGEMENT_STYLES = [
    "конкретный вопрос из личного опыта читателя (например: «вы уже пробовали?», «как это меняет ваш процесс?»)",
    "провокационный тезис без вопроса (например: «мне кажется, это меняет правила игры» или «и это только начало»)",
    "личное сомнение + приглашение поспорить (например: «честно говоря, не уверена, что зайдёт массово — или я ошибаюсь?»)",
    "мягкий призыв поделиться опытом без вопросительного знака (например: «буду рада узнать, если у кого-то есть похожий кейс 👇»)",
]
...
def generate_post(article: dict) -> str:
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "Пиши исключительно на русском языке. Только кириллица, латиница для имён собственных и цифры."},
